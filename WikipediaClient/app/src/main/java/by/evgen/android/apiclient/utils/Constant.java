package by.evgen.android.apiclient.utils;

/**
 * Created by User on 29.01.2015.
 */
public class Constant {

    //TODO
    public static final String GEOSEARCH = "geosearch";
    public static final String QUERY = "query";
    public static final String PAGES = "pages";
    public static final String KEY = "key";
    public static final String UTF = "utf-8";
    public static final String TYPE = "text/html";
    private static final String LOAD = "Loading...";
    private static final String DBDATE = "wikidate";
    private static final String DBNAME = "name";
    private static final String MEMBER = "categorymembers";
    private static final String MOBILE = "mobileview";
    private static final String SECTIONS = "sections";
    private static final String DBID = "_id";
    private static final String EMPTY = "";
    private static final String RESPONSE = "response";
    private static final String THUMBNAIL = "thumbnail";
    private static final String LIKED = "liked";
    private static final String ITEMS = "items";
    private static final String TEXT = "text";
    private static final String PARSE = "parse";
    private static final String WIKIRANDOM = "wikigrokrandom";
    private static final String SEARCH = "search";
    public static final String ACCOUNT_TYPE = "by.evgen.android.apiclient";


    public static String getEmpty() {
        return EMPTY;
    }
    public static String getLoad() {
        return LOAD;
    }
    public static String getDbDate() {
        return DBDATE;
    }
    public static String getDbName() {
        return DBNAME;
    }
    public static String getDbid() {
        return DBID;
    }
    public static String getMember() {
        return MEMBER;
    }
    public static String getSections() {
        return SECTIONS;
    }
    public static String getMobile() {
        return MOBILE;
    }
    public static String getResponse() {
        return RESPONSE;
    }
    public static String getThumbnail() {
        return THUMBNAIL;
    }
    public static String getLiked() {
        return LIKED;
    }
    public static String getItems() {
        return ITEMS;
    }
    public static String getText() {
        return TEXT;
    }
    public static String getParse() {
        return PARSE;
    }
    public static String getWikirandom() {
        return WIKIRANDOM;
    }
    public static String getSearch() {
        return SEARCH;
    }

}
